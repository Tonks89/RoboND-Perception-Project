...
# ----------------------> Exercise-2 TODOs:
    ...
    passthrough2 = cloud_filtered.make_passthrough_filter() # to filter deposit box edges
    filter_axis2 = 'y'
    ...
    ec.set_ClusterTolerance(0.05) # dist. threshold
    ec.set_MinClusterSize(30)
    ec.set_MaxClusterSize(7000) #2500 
...
# ----------------------> Exercise-3 TODOs:
    ...
    # Comment to run/visualize image processing steps only \!/
    try:
...
def pr2_mover(detected_objects): 
    ...
    for l_object in object_list_param: # For each item in pick list
        ...
        flag_detection = 0 # flag to signal: detection = 1, no detection = 0
	...
	for object in detected_objects: # scan through detected objects
            if object.label == object_name.data: # if desired obj. detected: compute pick pose, otherwise skip. 
            # Note: in case 2 obj. detected w/ same tag, it takes the last one --> to improve.
                flag_detection = 1
        ...
        # if the object was detected, add its properties to dictionary
        if flag_detection == 1:
